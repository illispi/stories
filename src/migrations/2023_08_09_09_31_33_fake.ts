...
export async function up(db: Kysely<any>): Promise<void> {
	await db.schema
		.createTable("Personal_questions_fake")
		...
		.addColumn("describe_hospital", "text", (col) =>
			col.check(
				sql`NOT (hospitalized_on_first AND describe_hospital IS NULL) `,
			),
		)
		...
		.addColumn("prodromal_depression", "boolean", (col) =>
			col.check(
				sql`NOT (prodromal_symptoms AND prodromal_depression IS NULL) `,
			),
		)
		...
		.addColumn("side_effs_movement_effects", "boolean", (col) =>
			col.check(
				sql`NOT (had_side_effs AND side_effs_movement_effects IS NULL)`,
			),
		)
		...
		.addColumn("disorganized_thinking", "boolean", (col) =>
			col.check(
				sql`NOT (cognitive_symptoms AND disorganized_thinking IS NULL)`,
			),
		)
	...
	await db.schema
		.createTable("Their_questions_fake")
		.addColumn("id", "serial", (col) => col.primaryKey())
...
export async function down(db: Kysely<any>): Promise<void> {
	await db.schema.dropTable("Their_questions_fake").ifExists().execute();
	await db.schema.dropTable("Personal_questions_fake").ifExists().execute();
}
